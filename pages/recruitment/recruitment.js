...
Page({
  ...
  onLoad: function(options) {
    const that = this;
    this.getdata(null, null, null, that.data.page)
  },
  ...
  onReachBottom: function() {
    ...
    if (that.data.flag == 0) {
      ...
      that.getdata(null, null, null, that.data.page);
    }
    ...
    if (that.data.flag == 1) {
      ...
      that.getdata(that.data.city == '全部' ? '' : that.data.city, date, that.data.education == '全部' ? '' : that.data.education, that.data.page1);
    }
  ...
  bindRegionChange1(e) {
    const that = this;
    if (that.data.city == '地区') {
      that.setData({
        city: ''
      })
    }

    if (that.data.education == '学历') {
      that.setData({
        education: ''
      })
    }
    ...
    this.setData({
      date: that.data.array1[e.detail.value],
      flag: 1,
      page1: 1,
      lists1: []
    })
    ...
    this.getdata(that.data.city == '全部' ? '' : that.data.city, date, that.data.education == '全部' ? '' : that.data.education, that.data.page1);
  },
  bindRegionChange2(e) {
    const that = this;
    if (that.data.date == '发布时间') {
      that.setData({
        date: ''
      })
    }
    ...
    this.setData({
      education: that.data.array[e.detail.value],
      flag: 1,
      page1: 1,
      lists1: []
    })
  ...
  onDetailTap: function(e) {
    ...
    wx.navigateTo({
      url: '/pages/recruitment/detail/index?id=' + id,
    })
  },
  ...
  getdata: function(city, date, education, page) {
    const that = this;
    wx.showLoading({
      title: '加载中',
    })
    ...
    wx.request({
      ...
      success: function(e) {
        ...
        if (that.data.flag == 0) {
          that.setData({
            lists: that.data.lists.concat(e.data.ongoing.data).concat(e.data.over.data)
          })
        } else if (e.data.over.max) {
          wx.showToast({
            title: '没有更多了',
            icon: 'none'
          })
        ...
        if (that.data.flag == 1) {
          that.setData({
            lists1: that.data.lists1.concat(e.data.ongoing.data).concat(e.data.over.data)
          })
        } else if (e.data.over.max) {
          wx.showToast({
            title: '没有更多了',
  ...
  searchtext: function() {
    ...
    wx.request({
      ...
      success:function(e){
        wx.hideLoading();
        that.setData({
          lists2: that.data.lists2.concat(e.data.ongoing.data).concat(e.data.over.data)
        })
      }
    })
  },
