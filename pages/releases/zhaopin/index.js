...
Page({
  ...
  xueli: function(e) {
    const that = this;
    this.setData({
      xueli: that.data.xuelis[e.detail.value]
    })
  },
  place: function(e) {
    console.log(e)
  ...
  confirm: function(e) {
    ...
    const param = {
      ...
      recruit: JSON.stringify({
        'img1': that.data.pngs,
        'ftitle': that.data.name,
        'title': that.data.job,
        'price': that.data.place,
        'linkurl': that.data.xueli,
        'relative': that.data.major,
        'author': that.data.money,
        'seotitle': that.data.url,
        'keyword': that.data.date,
        'description': that.data.workdata,
      })
    }
    wx.showLoading({
      title: '发布招聘中',
    ...
    if (that.data.pngs=='') {
      wx.request({
        url: app.api + 'sufe/user/examineRecruit',
        header: {
      ...
      wx.uploadFile({
        url: app.api + 'sufe/user/examineRecruit', // 仅为示例，非真实的接口地址
        filePath: that.data.pngs,
        header: {
          'content-type': 'application/x-www-form-urlencoded'
        },
        name: 'file',
  ...
  bindMultiPickerChange: function (e) {
    ...
    that.setData({
      ...
      place: that.data.multiArray[1][e.detail.value[1]]
    })
  },
